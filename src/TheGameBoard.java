...
public class TheGameBoard {
	...
	public TheGameBoard(List<Naming> players) {
		...
		for (int x = 0; x < ROWS; x++) {
			...
			PositionNow = new HashMap<Naming, Integer>();
			for (Naming player : players) {
				// The key is the player on the board the 0 is the starting position
				PositionNow.put(player, 0);
			}
	...
	public boolean movePlayer(Naming player, int Squares) {
		...
		if (PositionNext >= 100) {
			// Put the players on 100 once they reach 100 or more
	...
	public String toString() {
		...
		// Starts from the final row and we decrement
		for (int x = ROWS - 1; x >= 0; x--) {
			for (int y = 0; y < COLUMNS; y++) {
				if (oddRow) {
					...
					for (Naming Template : PositionNow.keySet()) {
						// For the players to go from right to left
					...
					String PlayerLocation = "";
					for (Naming Template : PositionNow.keySet()) {
						// For players going from right to left
			...
			// Once finished with printing a row then you switch rows
